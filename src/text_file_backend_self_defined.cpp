/*!
 * \file   text_file_backend_self_defined.cpp
 * \author Soley Ran
 * \date   07.06.2025
 *
 */
...
struct text_file_backend_self_defined::implementation
{
    //! File open mode
...
BOOST_LOG_API void text_file_backend_self_defined::construct(
...
BOOST_LOG_API void text_file_backend_self_defined::set_rotation_size(uintmax_t size)
{
    m_pImpl->m_FileRotationSize = size;
}

//! The method sets the maximum time interval between file rotations.
BOOST_LOG_API void text_file_backend_self_defined::set_time_based_rotation(time_based_rotation_predicate const& predicate)
{
    m_pImpl->m_TimeBasedRotation = predicate;
}

//! The method allows to enable or disable log file rotation on sink destruction.
BOOST_LOG_API void text_file_backend_self_defined::enable_final_rotation(bool enable)
{
...
BOOST_LOG_API bool text_file_backend_self_defined::is_valid_number(const std::string& str) 
{  
    try {  
        std::stoi(str);  
        return true;  
...
BOOST_LOG_API int text_file_backend_self_defined::get_next_filename_counter(const filesystem::path& directory) 
{  
    int cur_max_filename_counter = -1;
    std::unordered_set<int> filename_counter_set;
    for (filesystem::directory_iterator it(directory); it != filesystem::directory_iterator(); ++it)
    {
        if (filesystem::is_regular_file(it->status()))
        {
            ...
            if (filename.size() >= 5)
            {
                std::string first_five = filename.substr(0, 5);
                if (is_valid_number(first_five))
                {
                    filename_counter_set.emplace(std::stoi(first_five));
                    if(std::stoi(first_five) > cur_max_filename_counter)
                        cur_max_filename_counter = std::stoi(first_five);
                }
            }
        }
    }
    int next_filename_counter = 0;
    if(cur_max_filename_counter < 99999)
    {
        next_filename_counter = cur_max_filename_counter+1;
    }
    else
    {
        while(filename_counter_set.count(next_filename_counter))
            ++next_filename_counter;
    }
    return next_filename_counter;
}

BOOST_LOG_API path_string_type text_file_backend_self_defined::get_new_file_name()
{
    std::string modify_time = convert_time_to_string(time(nullptr), "%Y-%m-%d-%H-%M-%S");

    int next_filename_counter = get_next_filename_counter(m_pImpl->m_StorageDir);
    std::stringstream ss;  
    ss << std::setw(5) << std::setfill('0') << next_filename_counter;  
    std::string formatted_number = ss.str();  

    path_string_type new_file_name = formatted_number + "-" + modify_time + ".log";
...
BOOST_LOG_API void text_file_backend_self_defined::consume(record_view const& rec, string_type const& formatted_message)
{
    ...
    if (!m_pImpl->m_File.is_open())
    {
        ...
        if (!use_prev_file_name)
            ...
            new_file_name = m_pImpl->m_StorageDir / filesystem::path(get_new_file_name());
        else
...
BOOST_LOG_API void text_file_backend_self_defined::set_file_name_pattern_internal(filesystem::path const& pattern)
{
...
void text_file_backend_self_defined::close_file()
{
    if (m_pImpl->m_File.is_open())
    {
...
BOOST_LOG_API void text_file_backend_self_defined::rotate_file()
{
...
BOOST_LOG_API void text_file_backend_self_defined::set_open_mode(std::ios_base::openmode mode)
{
...
BOOST_LOG_API void text_file_backend_self_defined::set_file_collector(shared_ptr< file::collector > const& collector)
{
    m_pImpl->m_pFileCollector = collector;
}

//! The method sets file open handler
BOOST_LOG_API void text_file_backend_self_defined::set_open_handler(open_handler_type const& handler)
{
    m_pImpl->m_OpenHandler = handler;
}

//! The method sets file close handler
BOOST_LOG_API void text_file_backend_self_defined::set_close_handler(close_handler_type const& handler)
{
...
BOOST_LOG_API filesystem::path text_file_backend_self_defined::get_current_file_name() const
{
    return m_pImpl->m_FileName;
}

//! Performs scanning of the target directory for log files
BOOST_LOG_API uintmax_t text_file_backend_self_defined::scan_for_files(file::scan_method method, bool update_counter)
{
    if (m_pImpl->m_pFileCollector)
    {
