/*!
 * \file   text_file_backend_self_defined.hpp
 * \author Soley Ran
 * \date   07.06.2025
 *
...
#ifndef TEXT_FILE_BACKEND_SELF_DEFINED_HPP
#define TEXT_FILE_BACKEND_SELF_DEFINED_HPP
 ...
 */
class text_file_backend_self_defined :
...
public:
     ...
     */
    BOOST_LOG_API text_file_backend_self_defined();
     ...
     */
#ifndef BOOST_LOG_DOXYGEN_PASS
    BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_CALL(text_file_backend_self_defined, construct)
#else
    template< typename... ArgsT >
    explicit text_file_backend_self_defined(ArgsT... const& args);
#endif
...
private:
    ...
    BOOST_LOG_API std::string convert_time_to_string(const std::time_t& tt,const std::string& format);

    BOOST_LOG_API bool is_valid_number(const std::string& str);

    BOOST_LOG_API int get_next_filename_counter(const filesystem::path& directory);

    typedef filesystem::path::string_type path_string_type;
    typedef path_string_type::value_type path_char_type;
    BOOST_LOG_API path_string_type get_new_file_name();
};
